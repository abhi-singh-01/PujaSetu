/**
 * Generates india-locations.json with all states/UTs and districts.
 * Run: node src/scripts/generate-locations.js
 */
const fs = require('fs');
...
const RAW = {
  ...
  Assam: ['Baksa', 'Barpeta', 'Bongaigaon', 'Cachar', 'Darrang', 'Dhemaji', 'Dhubri', 'Dibrugarh', 'Goalpara', 'Golaghat', 'Hailakandi', 'Jorhat', 'Kamrup', 'Karimganj', 'Nagaon', 'Sivasagar', 'Sonitpur', 'Tinsukia'],
  ...
  Chhattisgarh: ['Balod', 'Baloda Bazar', 'Bastar', 'Bemetara', 'Bilaspur', 'Durg', 'Janjgir-Champa', 'Korba', 'Mahasamund', 'Raipur', 'Rajnandgaon', 'Surguja'],
...
const states = Object.entries(RAW).map(([name, districts], idx) => ({
  code: name.replace(/\s+/g, '_').toUpperCase().slice(0, 10),
  name,
  districts: districts.map((d) => ({
    code: d.replace(/\s+/g, '_').toUpperCase().slice(0, 12),
    name: d,
    cities: [`${d} City`, `${d} Town`, `${d} Rural`],
  })),
}));

const output = { country: 'India', states };
const outPath = path.join(__dirname, '../data/india-locations.json');
fs.mkdirSync(path.dirname(outPath), { recursive: true });
fs.writeFileSync(outPath, JSON.stringify(output, null, 2));
console.log(`Generated ${states.length} states to ${outPath}`);
